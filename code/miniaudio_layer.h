...
ma_uint32 read_and_mix_pcm_frames_f32(ma_decoder* pDecoder, float* pOutputF32, ma_uint32 frameCount)
{
	...
	*/
	...
	while (totalFramesRead < frameCount) {
		...
		result = ma_data_source_read_pcm_frames(pDecoder, temp, framesToReadThisIteration, &framesReadThisIteration);
...
// frameCount frames.
void miniaudio_callback(ma_device* device, void* output, const void* input, ma_uint32 frame_count)
{
...
int main(int argc, char** argv)
{
	...
	decoderConfig = ma_decoder_config_init(ma_format_f32, 2, SAMPLE_RATE);
	for (iDecoder = 0; iDecoder < g_decoderCount; ++iDecoder) {
    	...
    	ma_data_source_set_looping(&g_pDecoders[iDecoder], MA_TRUE);
